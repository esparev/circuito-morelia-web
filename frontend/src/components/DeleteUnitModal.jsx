...
const DeleteUnitModal = (props) => {
  ...
  const deleteUnit = async (url, config) => {
    ...
    await axios
      .delete(url, config)
      .then((res) => {
        root.render(<SuccessAlert successMessage={'¡Unidad eliminada exitosamente!'} />);
        setTimeout(() => {
      ...
      .catch((error) => {
        root.render(
          <ErrorAlert errorMessage={'¡Ups!, Hubo un error al eliminar la unidad.'} />
        );
        setTimeout(() => {
          document.querySelector('.alert__container').classList.remove('animate__slideInDown');
          document.querySelector('.alert__container').classList.add('animate__slideOutUp');
  ...
  const handleSubmit = (event) => {
    ...
    deleteUnit(`${envConfig.apiUrl}/units/${number}`, authConfig);
    hideModal();
  };
    ...
    <div className='delete__modal'>
      <div className='modal__container'>
        <div className='modal__header'>
          <h2 className='modal__title'>Eliminar Unidad</h2>
          <svg
